...
// --- PERLIN NOISE (klasický, bez přepsání Math.random) ---
class Perlin {
  constructor() {
...
// --- GPS → SEED (bez přepsání Math.random) ---
...
// --- SEEDOVANÝ RANDOM (bez poškození Math.random) ---
...
function generateArt(lat, lon) {
  ...
  const rand = createSeededRandom(seed);  // Lokální random

  const T = {
    scale: 0.001 + (lat % 1) * 0.002,
    octaves: 4 + Math.floor(Math.abs(lon % 1) * 4),
    rotation: (lat * 17.77) % 360,
    warp: Math.abs(Math.sin(seed * 0.00001)) * 1.8,
    grain: 0.6 + (lon % 1) * 0.4,
    vignette: 0.5 + (lat % 1) * 0.5,
    glitchChance: (seed % 100) / 100,
    shape: seed % 5
  };
  ...
  for (let y = 0; y < h; y++) {
    for (let x = 0; x < w; x++) {
      ...
      // Warping
      const wx = Math.sin(fy * 0.015) * T.warp * 35;
      const wy = Math.cos(fx * 0.015) * T.warp * 35;

      // Šum
      let n = 0, amp = 1, freq = 1;
      for (let o = 0; o < T.octaves; o++) {
        n += perlin.noise((rx + wx) * T.scale * freq + seed, (ry + wy) * T.scale * freq) * amp;
        amp *= 0.5;
        freq *= 2;
      }
      ...
      // Tvar
      const dist = Math.hypot(fx, fy) / rad;
      if (T.shape === 0) n *= (1 - dist * 0.7);
      else if (T.shape === 1) n = Math.abs(n - 0.5) < 0.03 ? 1 : 0.1;
      else if (T.shape === 2) n = (Math.sin((rx + ry) * 0.02) + 1) / 2;
      else if (T.shape === 3) n = n > 0.6 ? 1 : 0.2;
      else n = n;

      // Vinětace
      n *= (1 - Math.pow(dist, 1.5) * T.vignette);

      // Posterizace
      n = Math.floor(n * 7) / 7;

      // Zrnitost
      n += (rand() - 0.5) * T.grain * 0.35;
      n = Math.max(0, Math.min(1, n));

      // Glitch
      if (rand() < T.glitchChance * 0.05) n = rand();
  ...
  const shapes = ['ORGANIC', 'LINES', 'WAVES', 'BLOCKS', 'CHAOS'];
  statusEl.textContent = `SHAPE:${shapes[T.shape]} | OCT:${T.octaves} | SEED:${seed.toString(16).slice(0,6)}`;
}
